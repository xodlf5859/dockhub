							...
							addEventButton : {
								...
								click : function() {
									...
									window.open('<%=request.getContextPath()%>/mypage/scheduler/registForm.do',
											'일정등록',"width=800,height=700,left="+_left+",top="+_top);
								}
							}
						},
	                ...
	                <tbody>
	                    <c:if test="${!empty commingScheList }">
	                        <c:forEach var="comminglist" items="${commingScheList }" varStatus="vs">
	                            <tr>
	                                <td>${comminglist.mber_schdul_nm }</td>
	                                <td>${comminglist.mber_schdul_cn } </td>
	                                <td>
	                                    <c:set var="bgnde" value="${comminglist.mber_schdul_bgnde }" />
	                                    ${fn:substring(bgnde,0,16) }
	                                </td>
	                            </tr>
	                        </c:forEach>
	                    </c:if>
