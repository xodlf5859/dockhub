...
function loadRepositoryList() {
	var prjct_sn = $('input[id="prjctSn"]').val();
// 	var prjct_sn = $('#prjctSn').val();
	console.log(contextPath)
	console.log(prjct_sn)
	$.ajax({
		url: contextPath + '/repo/repo?prjct_sn=' + prjct_sn,
		method: 'get',
...
function saveGitRepo(){
	...
	var prjct_sn = $('input[id="prjctSn"]').val();
	$.ajax({
		url: contextPath + '/repo/regist.do?prjct_sn=' + prjct_sn,
		method: 'get',
		data : {
			'repo_url' :repo_url 
		},
...
function removeGit(){
	var prjct_sn = $('input[id="prjctSn"]').val();
	$.ajax({
		url: contextPath + '/repo/remove.do?prjct_sn=' + prjct_sn,
		success: function(){
			removeButton();
...
// 			fileNavBody.append("<xmp style='word-wrap: break-word; white-space: pre-wrap; background-color: #353535; color: white; border: 3px solid gray;'>" + contents + "</xmp>")
			...
			var basePath = $('<a '+ (path? 'class="filename-text"' : '') +' data-type="dir" onclick="location.reload();">'+gitRepoUrl+'</a>');
